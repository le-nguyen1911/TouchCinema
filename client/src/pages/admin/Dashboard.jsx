    ...
    <>
      ...
      <div className="relative flex flex-wrap gap-6 mt-4 max-w-5xl">
          ...
          {dashboardData.activeShows.map((show)=>(
            <div key={show._id} className="w-55 rounded-lg overflow-hidden h-full pb-3 bg-primary/30 border border-primary/20 hover:-translate-y-1 transition-all duration-300">
              <img src={show.movie.poster_path} alt="pst" className="h-60 w-full object-cover"/>
              <p className="font-medium p-2 truncate">{show.movie.title}</p>
              <div className="flex items-center justify-between px-2">
                  <p className="text-lg font-medium">{show.showPrice}{CURRENCY}</p>
                  <p className="flex items-center gap-1 tex-sm text-gray-400 mt-1 pr-1">
